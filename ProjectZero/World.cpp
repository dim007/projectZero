...
World::World(sf::RenderWindow& window, FontHolder& fonts)
	...
	mSceneGraph(),
	mSceneLayers(),
	mWorldBounds(0.f, 0.f, 10000.f, 1000), //world(left x pos, top y pos, max width, max height)
	mSpawnPosition(mWorldView.getSize().x / 2.f, 80.f),
...
void World::loadTextures() {
	...
	mTextures.load(Textures::Enemies, "textures/Enemies.png");
	mTextures.load(Textures::Attack, "zero_sprites/attack.png");
	mTextures.load(Textures::Animations, "zero_sprites/master_sprites.png");

	mTextures.load(Textures::Bullet, "textures/Bullet.png");
	mTextures.load(Textures::Missile, "textures/Missile.png");
	mTextures.load(Textures::HealthRefill, "textures/Pickups.png");
...
void World::buildScene() {
	...
	std::unique_ptr<Platform> platformSprite(new Platform(Platform::Highway, mTextures, sf::IntRect(0, 0, 1500, 210)));
	...
	std::unique_ptr<playerRobot> player(new playerRobot(Robot::Zero, mTextures, mFonts));
...
void World::update(sf::Time dt) { 
	...
	while (!mCommandQ.empty() && startTime >= (sf::seconds(2) + sf::seconds(1.75))) { //no commands till startTime and spawnTime
	...
	mPlayerPosition = mPlayerRobot->getWorldPosition();
	guideMissiles();
}
...
void World::spawnEnemies() {
	...
	while (!mEnemySpawnPoints.empty() && (mEnemySpawnPoints.back().y > getBattlefieldBounds().top
		&& mEnemySpawnPoints.back().x < getBattlefieldBounds().width + getBattlefieldBounds().left - 150.f)) {

		SpawnPoint spawn = mEnemySpawnPoints.back(); //get the closest enemy to us
		std::unique_ptr<EnemyRobot> enemy(new EnemyRobot(spawn.type, mTextures, mFonts, mPlayerPosition));
...
void World::addEnemies() {
	...
	addEnemy(Robot::slashSuit, 900.f, 400.f);
	addEnemy(Robot::Drone, 1100.f, 200.f);
	addEnemy(Robot::Drone, 1500.f, 200.f);
	addEnemy(Robot::slashSuit, 1100.f, 400.f);
	addEnemy(Robot::slashSuit, 1500.f, 400.f);
...
void World::handleCollisions() {
	...
	for (SceneNode::Pair pair : collisionPairs) {
		if (matchesCategories(pair, Category::PlayerRobot, Category::EnemyRobot)) {
			auto& player = static_cast<playerRobot&>(*pair.first);
			auto& enemy = static_cast<EnemyRobot&>(*pair.second);
			sf::Vector2f ePos = enemy.getWorldPosition();
			sf::Vector2f pPos = player.getWorldPosition();
			float xDiff = pPos.x - ePos.x;
			float yDiff = pPos.y - ePos.y;
			sf::FloatRect pRect = player.getBoundingRect();
			sf::FloatRect eRect = enemy.getBoundingRect();

			//horizontal collision
			if ((pPos.y + pRect.height / 2.f >= ePos.y - eRect.height / 2.f
			|| pPos.y - pRect.height / 2.f <= ePos.y + eRect.height / 2.f)
			&& std::abs(xDiff) < eRect.width / 2.f + pRect.width / 2.f) {
				if (xDiff < 0) //player is on right side of enemy
					player.setPosition(ePos.x - eRect.width / 2.f - pRect.width / 2.f
						, player.getWorldPosition().y);
				else if (xDiff > 0) //player on left side
					player.setPosition(ePos.x + eRect.width / 2.f + pRect.width / 2.f
						, player.getWorldPosition().y);
			}
			//vert collision
			if ((pPos.y + pRect.height / 2.f <= ePos.y - eRect.height / 2.f
			|| pPos.y - pRect.height / 2.f >= ePos.y + eRect.height / 2.f)
			&& std::abs(yDiff) < eRect.height / 2.f + pRect.height / 2.f) {
				if (xDiff < 0) //player is on top
					player.setPosition( player.getWorldPosition().x,
						ePos.y - eRect.height / 2.f - pRect.height / 2.f);
				else if (xDiff > 0) //player on bottom
					player.setPosition( player.getWorldPosition().x,
						ePos.y + eRect.height / 2.f + pRect.height / 2.f);
			}
		}
		...
		if (matchesCategories(pair, Category::EnemyRobot, Category::AlliedAttack)) {
			auto& robot = static_cast<EnemyRobot&>(*pair.first);
			auto& attack = static_cast<Attack&>(*pair.second);

			robot.damage(attack.getDamage());
		}
		if (matchesCategories(pair, Category::PlayerRobot, Category::EnemyAttack)) {
			auto& player = static_cast<playerRobot&>(*pair.first);
			auto& attack = static_cast<Attack&>(*pair.second);

			if(!player.isTakingDamage)
				player.damage(attack.getDamage());
			player.isTakingDamage = true;
			if (attack.getWorldPosition().x > player.getWorldPosition().x)
				player.damageDirection = -1.f;
			else
				player.damageDirection = 1.f;
		}
...
sf::FloatRect World::getBattlefieldBounds() const {
	...
	bounds.height += 100.f;
	bounds.top -= 100.f;
	bounds.width += 400.f; //compensate for right and left
	bounds.left -= 200.f;
	return bounds;
}
...
void World::setPlayerLives(int lives) {
	return mPlayerRobot->setLives(lives);
}